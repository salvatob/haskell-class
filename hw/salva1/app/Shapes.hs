...
data Shape =  Square IntPoint Int Color
            | Triangle IntPoint IntPoint IntPoint Color


-- My coordinates are in Ints,
-- (0,0) starts at top left corner instead of middle
-- And the unit is defined as a size of a small square, and relative to the window size
...
draw (Square (x, y) size clr) = 
    Color clr $
    Polygon $ translateCoordinates <$>
    [(x,y),(x+size,y),(x+size,y+size),(x,y+size)]

draw (Triangle a b c clr) = Color clr $
   Polygon $ translateCoordinates <$> [a,b,c]


drawOutline :: Shape -> Picture
drawOutline (Square (x, y) size _) = Scale 1.1 1.1 $ Color black $ Line $ translateCoordinates <$> [(x,y),(x+size,y),(x+size,y+size),(x,y+size),(x,y)]
drawOutline (Triangle a b c _) = Scale 1.1 1.1 $ Color black $ Line $ translateCoordinates <$> [a,b,c,a]
