...
prettyExpr =
  \case
    ...
    EVar v -> prettyName v
    EBinOp op l r -> prettyExpr l <+> prettyOp op <+> prettyExpr r
...
prettyStmt =
  \case
    ...
    SIf cond body -> "if" <+> parens (prettyExpr cond) <+> prettyStmt body

    SWhile cond body -> "while" <+> parens (prettyExpr cond) <+> prettyStmt body

    SIfElse cond thenBody elseBody ->
      "if"
        <+> parens (prettyExpr cond)
        <+> prettyStmt thenBody
        <+> "else"
        <+> prettyStmt elseBody

    SExpr expr -> prettyExpr expr <> ";"
    ...
    SFuncDef name params body ->
      "function"
        <+> prettyName name <> tupled (map prettyName params)
        <+> prettyStmt body
